...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		List<PointFeature> earthquakes = ParseFeed.parseEarthquake(this, earthquakesURL);
		int countOcean = 0;
		//count earthquake in each country
		for (Marker countryMarker:countryMarkers){
			int count = 0;
			for(PointFeature quake: earthquakes){
				if (isInCountry(quake, countryMarker)){
					//System.out.println((new LandQuakeMarker(quake)).getCountry());
					count ++;
				}
			}
			if (count > 0){
		...
		//count quakes in ocean
		for (PointFeature quake: earthquakes){
			if (!isLand(quake)){
					countOcean ++;
				}
			}
		System.out.println("Ocean quakes");
		System.out.println(countOcean);
	}
